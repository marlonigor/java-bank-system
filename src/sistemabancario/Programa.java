...
public class Programa {
    ...
    public static void main(String[] args) {
        ...
        do {
            Menu.menuGeral();
            opcaoMenuGeral = lerOpcaoInt(sc);

            switch (opcaoMenuGeral) {
                case 1: // Secao Conta
                    Menu.menuConta();
                    int opcaoMenuConta = lerOpcaoInt(sc);
                    ...
                    switch (opcaoMenuConta) {
                        ...
                        case 2: // Acessar Conta
                            ...
                            if (correntistaEncontrado == null) {
                                System.out.println(
                                        "Correntista nao encontrado. Crie um novo correntista ou escolha um existente.");
                            } else {
                                Menu.menuTipoConta();
                            }
                            ...
                            int opcaoTipoConta = lerOpcaoInt(sc);
                                ...
                                case 1: // Conta Corrente
                                    for (Conta conta : correntistaEncontrado.getContas()) {
                                        if (conta instanceof ContaCorrente) {
                                            ...
                                            System.out.println("Conta Corrente de numero " + contaEncontrada.getNumero()
                                                    + " encontrada para o CPF " + correntistaEncontrado.getCpf() + ".");
                                            break;
                                        }
                                    }
                                    break;
                                ...
                                case 2: // Conta Poupança
                                    for (Conta conta : correntistaEncontrado.getContas()) {
                                        if (conta instanceof ContaPoupanca) {
                                            contaEncontrada = conta;
                                            System.out.println("Conta Poupanca de numero " + contaEncontrada.getNumero()
                                                    + " encontrada para o CPF " + correntistaEncontrado.getCpf() + ".");
                                            break;
                                        }
                                    }
                                ...
                                case 3: // Conta Poupança
                                    for (Conta conta : correntistaEncontrado.getContas()) {
                                        if (conta instanceof ContaAplicacao) {
                                            contaEncontrada = conta;
                                            System.out.println("Conta Aplicacao de numero "
                                                    + contaEncontrada.getNumero() + " encontrada para o CPF "
                                                    + correntistaEncontrado.getCpf() + ".");
                                            break;
                                        }
                                    }
                                    break;
                            ...
                            if (contaEncontrada != null) {
                                ...
                                do {
                                    ...
                                    opcaoOperacao = lerOpcaoInt(sc);

                                    switch (opcaoOperacao) {
                                        case 1: // Depositar
                                            System.out.println("Insira o valor do deposito: ");
                                            double valorDeposito = lerDouble(sc);
                                            contaEncontrada.depositar(valorDeposito);

                                            break;
                                        case 2: // Sacar
                                            System.out.println("Insira o valor do saque: ");
                                            double valorSaque = lerDouble(sc);
                ...
                case 2:
                    ...
                    int opcaoMenuCorrentista = lerOpcaoInt(sc);
            ...
            switch (opcaoTipoConta) {
                case 1: // Conta Corrente
                    if (correntistaEncontrado.possuiConta(ContaCorrente.class)) {
                        ...
                        System.out.println("Conta Corrente de numero " + novaCC.getNumero() + " criada para: "
                                + correntistaEncontrado.getNome());
                    }
                    break;
                ...
                case 2: // Conta Poupanca
                    if (correntistaEncontrado.possuiConta(ContaPoupanca.class)) {
                        ...
                        System.out.println("Conta Poupança de numero " + novaCP.getNumero() + " criada para: "
                                + correntistaEncontrado.getNome());
                    }
                    break;
                ...
                case 3: // Conta Aplicação
                    if (correntistaEncontrado.possuiConta(ContaAplicacao.class)) {
                        ...
                        System.out.println("Conta Aplicacao de numero " + novaCA.getNumero() + " criada para: "
                                + correntistaEncontrado.getNome());
                    }
                    break;
        ...
        switch (opcaoTipoConta) {
            case 1: // Conta Corrente
                for (Conta conta : correntistaDestino.getContas()) {
                    if (conta instanceof ContaCorrente) {
                        contaDestino = conta;
                        System.out.println("Conta Corrente de numero " + contaDestino.getNumero()
                                + " encontrada para o CPF " + correntistaDestino.getCpf() + ".");

                        System.out.println("Insira o valor que sera transferido");
                        double valorTransferencia = lerDouble(sc);
                        ...
                        if (valorTransferencia > contaEncontrada.getSaldo()) {
                            ...
                            System.out.println("Transferencia da " + contaEncontrada.getTipoConta() + "de numero "
                                    + contaEncontrada.getNumero() + " realizada para a " + contaDestino.getTipoConta()
                                    + " de numero " + contaDestino.getNumero() + " no valor de R$ " + valorTransferencia
                                    + ".");
    ...
    private static int lerOpcaoInt(Scanner sc) {
        while (true) {
            try {
    ...
    private static double lerDouble(Scanner sc) {
        while (true) {
            try {
                String input = sc.nextLine().trim();
                // Replace comma with dot for compatibility if user types 10,50
                input = input.replace(",", ".");
                if (input.isEmpty())
                    continue;
                return Double.parseDouble(input);
            } catch (NumberFormatException e) {
                System.out.println("Valor invalido. Por favor, digite um valor numerico valido:");
            }
        }
    }
}
